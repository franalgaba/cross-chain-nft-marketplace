...
async def proxy_init(contract_defs):
    ...
    v1 = await starknet.deploy(contract_def=dummy_v1_def, constructor_calldata=[])
    v2 = await starknet.deploy(contract_def=dummy_v2_def, constructor_calldata=[])
    proxy = await starknet.deploy(
        contract_def=proxy_def, constructor_calldata=[v1.contract_address]
    )
    return (starknet.state, account1, account2, v1, v2, proxy)
...
async def after_upgrade(proxy_factory):
    ...
    # set value
    await signer.send_transaction(
        admin, proxy.contract_address, "set_value_1", [VALUE_1]
    )

    # upgrade
    await signer.send_transaction(
        admin, proxy.contract_address, "upgrade", [v2.contract_address]
    )
...
async def test_upgrade_from_non_admin(proxy_factory):
    ...
    await assert_revert(
        signer.send_transaction(
            non_admin, proxy.contract_address, "upgrade", [v2.contract_address]
        ),
...
async def test_set_admin(after_upgrade):
    ...
    # change admin
    await signer.send_transaction(
        admin, proxy.contract_address, "set_admin", [new_admin.contract_address]
    )
...
async def test_set_admin_from_non_admin(after_upgrade):
    ...
    await assert_revert(
        signer.send_transaction(
            non_admin, proxy.contract_address, "set_admin", [non_admin.contract_address]
        )
    )
